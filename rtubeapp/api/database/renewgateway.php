...
while($row = mysqli_fetch_array($result))
{
	...
	$pcur="$row[currency]";
	$ptax="$row[tax]";
	$pclvoucher="$row[pay_via_voucher]";
$total=$pprice+$ptax;
// "<option value='$id'>$pname | Price - $pcur $total </option>";
...
if($numrows==1) {
	...
	<input type="hidden" name="notify_url" value="http://www.sample.in/demo/cdecnyn/user/success.php?method=PAYPAL"> 
...
if($numrows==1) {
    ...
    <input type="hidden" name="ap_alerturl" value="http://www.sample.in/demo/cdecnyn/user/success.php?method=PAYZA">
...
<form action="https://solidtrustpay.com/handle.php" method="post">
    ...
    <input type="hidden" name="amount" value="<?php print $total ?>">
    <input type="hidden" name="notify_url" value="http://www.sample.in/demo/cdecnyn/user/success.php?method=SOLIDTRUSTPAY">
...
<?php
 $query="SELECT count(*) FROM  paymentgateway where name='Marketer Balance' and status=1"; 
...
if($numrows==1) {
	...
	<input type="button" class="btn btn-lg btn-primary btn-block" id="marketer" value="Use <?php if($pck!=3){ print $pclvoucher;} //print $pprice ?> Voucher" >
	
	<script>
	
	$("#marketer").click(function(){   $('#marketer_balance').show(); });
	$("#c_o_d").click(function(){ $('#marketer_balance').hide(); });
	</script>
...
<div id="marketer_balance" class="marketer_balance" style="display:none; max-width: 36%;margin-top: 10px; margin-left: auto; margin-right: auto;">
<div class="navbar-brand block">Enter Referal password to check balance</div>
 
            <!--<input type="hidden" placeholder="Username" id="username" class="form-control no-border" name="username" value="<?php echo $refname; ?>" required>-->
            <input type="hidden" placeholder="Pay" id="pay_amt" class="form-control no-border" name="pay_amt" value="<?php echo $pprice; ?>" required>
            <input type="hidden"  id="userid" class="form-control no-border" name="userid" value="<?php echo $userid; ?>" required>
        <input type="hidden" name="renewpck" id="renewpck" value="<?php print $id ?>">
         <div class="list-group-item">
         <input type="text" placeholder="Username" id="username" class="form-control no-border" name="username" value="<?php echo $refname; ?>" required>
          </div><br/>
         
          <div class="list-group-item">
            <input type="password" placeholder="Referal Password" id="password" class="form-control no-border" name="password" required>
          </div><br/>
      
        <button type="button" class="btn btn-lg btn-primary btn-block" onclick=" return check_balance();">Confirm to pay</button>
...
function check_balance(){
    ...
    var url = "checkbalance1.php"; // the script where you handle the form input.
	var uname=$('#username').val();
	var pwd=$('#password').val();
	var payamt=$('#pay_amt').val();
	var userid=$('#userid').val();
	var renewpck=$("#renewpck").val();
    ...
    function(data, status){
       ...
       {
       	 alert('Payment cannot process due to invalid account or insufficient voucher balance..!');		
       }
    });
